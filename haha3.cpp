...
const string DIR[3]={"turn left", "go straight", "turn right"};
...
int getDirNum(char x){return (x == 'D') ? 1 : ( (x == 'L') ? 0 : 2 );}
struct LOCATION{
...
struct CAR
{
    ...
    void AddState(int _roadPos , int _channel, int _dis, int _dir, int _nowSpeed){
        location = LOCATION(_roadPos, _channel, _dis);
        nowSpeed = _nowSpeed;
        dir = _dir;
    }

    void SetOff(int _time){
        setoffTime = _time;
    }
    ...
    void out(){
        ...
        cout << "\t";   location.out();
        cout << "\tThe car is going to " << DIR[dir] << endl;
...
int stateNumber;
void ReadCarState(){
    freopen("carstate.txt","r",stdin);
    scanf("%d", &stateNumber);
    for (int i = 0 ; i < stateNumber; i++ ){
        int carId, roadId, channel, toCross, dis, dir, nowSpeed;
        char dd[5];
        scanf("%d%d%d%d%d%s%d", &carId, &roadId, &channel, &toCross, &dis, dd, &nowSpeed);
        dir = getDirNum(dd[0]);

        carId = carIdToPos[carId];
        car[carId].AddState(getRoadPos(roadId, toCross) , channel, dis, dir, nowSpeed);
    }
    if (VGeler){
        for (auto &cc : car) cc.out();
    }
