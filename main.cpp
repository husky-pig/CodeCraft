...
struct EDGE
{
    ...
    EDGE() {}
    EDGE(int _eid, int _from, int _to, int _length, int _speed, int _channel, int _isDuplex) :
        eid(_eid), from(_from), to(_to), length(_length), speed(_speed), channel(_channel), isDuplex(_isDuplex)
        {
...
void AllInitialize(const vector<Road> &roads, const vector<Cross> &crosses, const vector<Task> &tasks,
                   vector<EDGE> &edges, vector<POINT> &points, vector< vector<int> > &carPath)
{
    ...
    map<int, int> point2Array;

    for(int i = 0; i < roads.size(); i++)
    {
        const auto &road = roads[i];
        edges.push_back(EDGE(road.Rid, road.from, road.to, road.length, road.speed, road.channel, road.isDuplex));
        edge2Array[road.Rid] = edges.size() - 1;

        if(road.isDuplex)
        {
            edges.push_back(EDGE(road.Rid, road.to, road.from, road.length, road.speed, road.channel, road.isDuplex));
        }
    }
    ...
    for(int i = 0; i < crosses.size(); i++)
    {
        ...
        point2Array[cross.Cid] = points.size() - 1;
...
int RunTest(vector< vector<int> > &carPath)
{
    ...
    AllInitialize(G.roads, G.crosses, tasks, edges, points, carPath);
    cout <<"end init" <<endl;
